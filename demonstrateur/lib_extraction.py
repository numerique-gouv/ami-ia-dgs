...
def from_pdf_to_mrv_format(file, Colonnes: list, mapping: dict,from_path=False) -> (pd.DataFrame):
    ...
    df_mrv['TYPE_DECLARANT'] = TYPE_DECLARANT
    return(df_mrv)
...
def from_xml_to_mrv_format(PATH, Colonnes, mapping):
    """
    ...
    Args:
        PATH (str): le chemin du fichier xml à transformer
    ...
    try:
    ...
    except:# la version de base ne fonctionne pas
        try :  # on gère le cas de l'encodage avec une exception de manière provisoire
        ...
        except : # on renvoie une table vide
            df_table= df_table
        
        
    # Ajout du Type et du Numero de déclaration   
    try : #Structure de base
        with open(PATH, 'r') as f:
            xml_file = f.read()
    ...
    except : # Si la structure de base ne fonctionne pas
        try : # on essaie l'encodage
            with open(PATH, 'r') as f:
                xml_file = f.read()
                content = base64.b64decode(xmltodict.parse(xml_file))
                TYPE = content[
                    'soap:Envelope']['soap:Body']['ConsulterSignalementResponseV3']['ns4:signalementRetourList']['ns4:signalement'][0]['ns8:typeSignalement']
                NUMERO = content['soap:Envelope']['soap:Body']['ConsulterSignalementResponseV3'][
                    'ns4:signalementRetourList']['ns4:signalement'][0]['ns8:declaration']['ns14:referenceDeclaration']
                
                    
        except : #si m'encodage ne fonctionne pas on renvoie du vide ou les infos présentent dans le nom de fichier
    ...
    # Ajout du Type de Declarant :
    try : #Structure de base
        with open(PATH, 'r') as f:
            xml_file = f.read()
            data_dict = xmltodict.parse(xml_file)
            if '(ps)' in data_dict['ClinicalDocument']['title'].lower():
                TYPE_DECLARANT = 'Etablissements et professionnels de santé'
            else :
                TYPE_DECLARANT = 'Citoyen'
    except :
        try :# Encodage 64bit
            xml_file = f.read()
            content = base64.b64decode(xmltodict.parse(xml_file))
            if content['soap:Envelope']['soap:Body']['ConsulterSignalementResponseV3']['ns4:signalementRetourList']['ns4:signalement'][0]['ns8:declaration']['ns14:profilDeclarant']=='USAGER':                     
                TYPE_DECLARANT = 'Citoyen'
            else :
                TYPE_DECLARANT = 'Etablissements et professionnels de santé'
        except: #vide
    ...
    # On applique le mapping
    df_mrv = create_fus(df_table, mapping)
    df_mrv['CLASSIFICATION'] = [elt.encode('utf-8').decode('utf-8') for elt in df_mrv['CLASSIFICATION'].fillna('NON RENSEIGNE').tolist()]
    df_mrv['NUMERO_DECLARATION'] = NUMERO
    df_mrv['TYPE_VIGILANCE'] = TYPE
    df_mrv['TYPE_DECLARANT'] = TYPE_DECLARANT
