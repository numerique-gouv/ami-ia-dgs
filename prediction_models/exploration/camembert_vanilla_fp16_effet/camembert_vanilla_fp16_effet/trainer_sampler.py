...
from torchsampler import ImbalancedDatasetSampler, ImbalancedDatasetSamplerVec
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if is_tpu_available():
            ...
            if self.balanced_sampler:
                ...
                if self.multilabel:
                    train_sampler = ImbalancedDatasetSamplerVec(self.train_dataset)
                else:
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Features:  # Trainer doesn't support IterableDataset (define sampler)
        ...
        ex = (self.examples[idx] if model_args.balanced_sampler else
              self.examples[self.current])   # idx to make balanced sampling work
        self.current += 1
...
class SmartCollator(DataCollator):
    ...
    def collate_batch(self, batch: List[Features]) -> Dict[str, torch.Tensor]:
        ...
        return {"input_ids": torch.tensor(batch_inputs, dtype=torch.long),
                ...
                "labels": torch.tensor(labels, dtype=torch.long) # float to support multilabel
                }
...
def compute_metrics_multi(p: EvalPrediction) -> Dict:
    ...
    my_preds_label = (preds > 0.5).astype(int).sum(axis=1)
    my_preds_true = p.label_ids.sum(axis=1)
    balanced_accuracy = balanced_accuracy_score(my_preds_true, my_preds_label)
    return {"acc": accuracy, "b_acc": balanced_accuracy}
...
if __name__ == "__main__":
    ...
    train_sentences = load_train_data(path="data/train_test/train.csv",
    ...
    valid_sentences = load_dev_data(path="data/train_test/test.csv")
