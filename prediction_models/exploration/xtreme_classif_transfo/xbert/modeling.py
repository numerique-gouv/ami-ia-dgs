...
@add_start_docstrings(
    """Cammebert Model transformer with a sequence classification head on top (a linear layer
  on top of the pooled output) e.g. for eXtreme Multi-label Classification (XMLC). """,
    CAMEMBERT_START_DOCSTRING,
)
class CamembertForXMLC(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels]``.
  ...
  """
    ...
    def __init__(self, config):
        super(CamembertForXMLC, self).__init__(config)
        self.num_labels = config.num_labels
