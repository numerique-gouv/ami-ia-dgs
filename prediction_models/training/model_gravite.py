...
def GRAVITE_ENC(x):
    """ Fonction pour encoder les 5 valeurs de la gravité avec des entier allant de 1 à 4. 
...
def prepare_data(df: pd.DataFrame, clean=True):
    ...
    pour l'inférence de la Gravité. Elle se compose des étapes suivantes :
    - suppression des lignes contenant les valeurs nulles
    - gestion des valeurs nulles
    - nettoyage des données textuelles (ponctuation etc.)
    ...
    if clean == True:
        ...
        for col in ['DESCRIPTION_INCIDENT', 'ETAT_PATIENT']:
...
def train_GRAVITE(df_train: pd.DataFrame, cv=3, save=False) -> (sk.pipeline.Pipeline):
    ...
    tfidf finetuné avec la librairie Optuna puis utilise un  SVM linéaire  combiné avec un classifier ordinale qui prend en compte 
    l'ordre des classes pour classifier les Gravités.
    ...
    pipeline = Pipeline([
        ('vect', preprocess),
        ('clf', OrdinalClassifier(CalibratedClassifierCV(
            LinearSVC(class_weight='balanced'), cv=cv, method='isotonic')))
    ])

    X = df_train[['DESCRIPTION_INCIDENT',
                  'ETAT_PATIENT', 'FABRICANT', 'CLASSIFICATION']]
    y = df_train.GRAVITE

    pipeline.fit(X, y)

    if save == True:
        joblib.dump(pipeline, os.path.join(MODEL_PATH, 'GRAVITE_model_last.sav'))
...
def evaluate_model(pipeline: sk.pipeline.Pipeline, df_test: pd.DataFrame) -> (float):
    """
    Renvoie le score adéquate pour le pipeline chargé. Dans le cas de la gravité, la balance accuracy est le score adéquate pour notre problème
    Entrées:
    ...
    X_test = df_test[['DESCRIPTION_INCIDENT',
                      'ETAT_PATIENT', 'FABRICANT', 'CLASSIFICATION']]
